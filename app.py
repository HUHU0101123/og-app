...
col1.markdown("**Ventas Totales:** Ingresos totales antes de descuentos y ajustes.")

col2.metric("Descuentos Aplicados", f"${filtered_df['Descuento del producto'].sum():,.0f}")
col2.markdown("**Descuentos Aplicados:** Total de descuentos otorgados en ventas.")

col3.metric("Ventas Netas", f"${ventas_netas:,.0f}")
col3.markdown("**Ventas Netas:** Ventas totales menos descuentos.")

col4.metric("Ventas Netas Después de Impuestos", f"${ventas_netas_despues_impuestos:,.0f}")
col4.markdown("**Ventas Netas Después de Impuestos:** Ventas netas menos impuestos del 19%.")

st.header("Métricas Adicionales")
col1, col2, col3, col4 = st.columns(4)

col1.metric("Cantidad de Órdenes", filtered_df['ID'].nunique())
col1.markdown("**Cantidad de Órdenes:** Número total de órdenes procesadas.")
